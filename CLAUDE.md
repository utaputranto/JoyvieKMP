...
- `core:mvi` — reusable MVI base. `BaseStateMachine<S : UiState, E : UiEvent, F : UiEffect>` (extends
  `ViewModel`) exposes a single `state: StateFlow<S>` + one-shot `effect: Flow<F>`; subclasses reduce
  via `setState { copy(...) }`, emit via `sendEffect(...)`, and route intents through `onEvent(event)`.
  Auto-wired into every `:feature:*:presentation` by `FeaturePresentationConventionPlugin`, so new
  state machines just extend it (see `OnboardingStateMachine`). No Compose dep (uses plain
  `lifecycle-viewmodel`). Naming: ViewModels that extend `BaseStateMachine` are named
  `XStateMachine`, not `XViewModel`.
